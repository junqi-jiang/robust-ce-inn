...
class OptSolver:
        ...
        for feat_idx in range(self.dataset.num_features):
            ...
            if self.dataset.feature_types[feat_idx] == DataType.DISCRETE:
                ...
                for var_idx in self.dataset.feat_var_map[feat_idx]:
                    ...
                    #self.model.addConstr(node_var[var_idx] == self.x[var_idx])  ### debug
                self.model.update()
        ...
        for i in range(1, self.inn.num_layers):
            ...
            for node in self.inn.nodes[i]:
                ...
                if i != (self.inn.num_layers - 1):
                    node_var[node.index] = self.model.addVar(lb=0, vtype=GRB.SEMICONT, name='n_' + str(node))
                    aux_var[node.index] = self.model.addVar(lb=-GRB.INFINITY, vtype=GRB.SEMICONT,
                                                            name='a_' + str(node))
                    self.model.update()
                ...
                # add output constraint: y = WX+b,
                    ...
                    node_var[node.index] = self.model.addVar(lb=-GRB.INFINITY, vtype=GRB.SEMICONT,
                                                             name='n_' + str(node))
    ...
    def set_objective(self, node_vars):
        ...
        for idx in node_vars[0].keys():
            this_obj_var = self.model.addVar(vtype=GRB.SEMICONT, lb=0, ub=1, name=f"obj_xd_{idx}")
            self.model.update()
