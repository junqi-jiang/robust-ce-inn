    ...
    for i in range(num_layers - 1):
        for node_from in nodes[i]:
            for node_to in nodes[i + 1]:
                # round by 4 decimals
                w_val = round(ws[i][node_from.index][node_to.index], 3)
...
def experiment_cf_properties(indices):
    """
    experiment for evaluating the quality of counterfactuals generated by a method.
    Metrics:
    1. Proximity: normalised L1
    2. Sparsity: normalised L0
    3. Gen-Validity: percentage of test instances that 1) have CEs
    4. m1-Validity: percentage of test instances that additionally 2) CEs are valid on m1
    5. m2-Validity: percentage of test instances that
    :param indices:
    :return:
    """
    pass
